...
%             Note, this is non-functional in this NDHist
        ...
        % % % % % MY STUFF!! Edges for Kp
        case 'yedges'
            yEdgesFlag = 1;
            yEdgesKp = varargin{k + 1};
            k = k + 1;
        case 'xedges'
            xEdgesFlag = 1;
            xEdgesKp = varargin{k + 1};
            k = k + 1;
        case 'numpoints'
            numPointsFlag = 1;
            numPointsMin = varargin{k + 1};
            k = k + 1;
...
% % % % % % My stuff - for kp edges
if xEdgesFlag
    edgesX = reshape(xEdgesKp, 1, length(xEdgesKp));
else
    edgesX=axisXY(1):binWidthX:axisXY(2);
end
if yEdgesFlag
    edgesY = reshape(yEdgesKp, 1, length(yEdgesKp));
else
...
if numPointsFlag
    N = hist3numpoints(x,y,edgesX,edgesY,numPointsMin);
else
...
edgesY2 = edgesY+binWidthY/2; % <- check this!
...
% % % % % My stuff
function allN = hist3numpoints(x,y,edgesX,edgesY,numPointsMin)
    allN = zeros(length(edgesY),length(edgesX));
    [~,binX] = histc(x,edgesX);
    for ii=1:length(edgesX)
        I = (binX==ii);
        N = histc(y(I),edgesY);
        N(N < numPointsMin) = NaN;
        allN(:,ii) = N';
    end
end 
% % % % %
