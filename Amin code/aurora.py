...
NAME = "Auroral_Electron_Flux_ANN_64x1_{}".format(int(time.time()))
...
dataset = dataset.drop(columns = ['Year','Day','Hour','Minute', 'ELE_TOTAL_ENERGY_FLUX_STD', \
...
day_sin = np.sin(np.sin(day/365)*2*np.pi)
day_cos = np.cos(np.sin(day/365)*2*np.pi)
aacgm_ltime_sin = np.sin(np.sin(sc_aacgm_ltime/24)*2*np.pi)
aacgm_ltime_cos = np.cos(np.sin(sc_aacgm_ltime/24)*2*np.pi)
...
#training plot
history_train = model.fit(normed_dataset_train, normed_dmsp_train_label, epochs = 1, validation_split = 0.2, callbacks = [tensorboard])
